...
router ospf
 ...
 network $lan_subnet1 area 0
 network $lan_subnet2 area 0
...
# Prompt for inputs
read -p "Enter first interface name (e.g., eno2np1) [eno2np1]: " interface1
interface1=${interface1:-eno2np1}
check_interface "$interface1"

read -p "Enter first interface LAN IP (e.g., 172.16.0.1/30) [172.16.250.1/30]: " lan_ip1
lan_ip1=${lan_ip1:-172.16.250.1/30}

read -p "Enter first interface LAN subnet (e.g., 172.16.0.0/30) [172.16.250.0/30]: " lan_subnet1
lan_subnet1=${lan_subnet1:-172.16.250.0/30}
lan_net_addr1=$(echo "$lan_subnet1" | cut -d'/' -f1)

# Prompt for second interface
read -p "Enter second interface name (e.g., eno1np0) [eno1np0]: " interface2
interface2=${interface2:-eno1np0}
check_interface "$interface2"

read -p "Enter second interface LAN IP (e.g., 172.16.250.5/30) [172.16.250.5/30]: " lan_ip2
lan_ip2=${lan_ip2:-172.16.250.5/30}

read -p "Enter second interface LAN subnet (e.g., 172.16.250.4/30) [172.16.250.4/30]: " lan_subnet2
lan_subnet2=${lan_subnet2:-172.16.250.4/30}
lan_net_addr2=$(echo "$lan_subnet2" | cut -d'/' -f1)

# For backward compatibility, keep original variables pointing to first interface
interface=$interface1
lan_ip=$lan_ip1
lan_subnet=$lan_subnet1
lan_net_addr=$lan_net_addr1

# Check if both interfaces exist and get current configuration
if ! ip link show "$interface1" &> /dev/null; then
    echo "❌ Interface $interface1 does not exist"
    exit 1
fi

if ! ip link show "$interface2" &> /dev/null; then
    echo "❌ Interface $interface2 does not exist"
    exit 1
fi

echo "✅ Found interfaces: $interface1, $interface2"
...
# Extract IP address and prefix for both interfaces
IP_ADDR1=$(echo "$lan_ip1" | cut -d'/' -f1)
PREFIX1=$(echo "$lan_ip1" | cut -d'/' -f2)
IP_ADDR2=$(echo "$lan_ip2" | cut -d'/' -f1)
PREFIX2=$(echo "$lan_ip2" | cut -d'/' -f2)

# Check if either interface already exists in the netplan file
INTERFACE1_EXISTS=$(sudo grep -q "^\s*$interface1:" "$NETPLAN_FILE" && echo "true" || echo "false")
INTERFACE2_EXISTS=$(sudo grep -q "^\s*$interface2:" "$NETPLAN_FILE" && echo "true" || echo "false")

if [ "$INTERFACE1_EXISTS" = "true" ] || [ "$INTERFACE2_EXISTS" = "true" ]; then
    echo "⚠️  One or both interfaces already exist in netplan configuration"
...
try:
    ...
    # Update or add the interface configurations
    if 'ethernets' not in config['network']:
        config['network']['ethernets'] = {}
    
    # Configure first interface
    config['network']['ethernets']['$interface1'] = {
        'dhcp4': False,
    ...
    config['network']['ethernets']['$interface2'] = {
        'dhcp4': False,
...
" || {
    ...
    echo "✅ Adding new interfaces $interface1 and $interface2 to netplan configuration"
    
    # Add the new interfaces to the existing configuration
    sudo python3 -c "
import yaml
import sys
...
try:
    ...
    # Add the new interface configurations
    if 'ethernets' not in config['network']:
        config['network']['ethernets'] = {}
...
echo "✅ $interface1 is now configured with static IP $lan_ip1 (no gateway, no DNS)"
echo "✅ $interface2 is now configured with static IP $lan_ip2 (no gateway, no DNS)"

# Wait a moment for network to stabilize after netplan apply
sleep 2
...
if [ -n "$INTERNET_IFACE" ]; then
    ...
    echo "   Please ensure you have internet connectivity before running this script." >&2
    exit 1
fi
...
detect_internet_interface() {
    ...
    if [ -n "\$INTERNET_IFACE" ]; then
        ...
        echo "2 - No default internet interface found." >&2
        exit 1
    fi
}
...
echo "|   NSB's Default Gateway: $lan_ip1                          |"
echo "|            NSB's Subnet: $lan_subnet1                      |"
echo "|          DHCP Server IP: $lan_ip1                          |"
echo "|        RADIUS Server IP: $lan_ip1                          |"
echo "|____________________________________________________________|"
echo "| INTERFACE 2 ($interface2):                                 |"
echo "|   NSB's Default Gateway: $lan_ip2                          |"
echo "|            NSB's Subnet: $lan_subnet2                      |"
echo "|          DHCP Server IP: $lan_ip2                          |"
echo "|        RADIUS Server IP: $lan_ip2                          |"
echo "|____________________________________________________________|"
