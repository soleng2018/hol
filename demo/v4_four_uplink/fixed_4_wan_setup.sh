...
echo "🚀 Fixed 4-WAN Network Setup Script"
echo "===================================="
echo "This script configures exactly 4 network uplinks using parameters from parameters.txt"
echo ""
...
create_frr_container() {
    local containerName="frr_fixed4wan"
    
    docker run -dt --name "$containerName" --network=host --privileged --restart=always \
    -v ./frr_fixed4wan.conf:/etc/frr/frr.conf:Z \
...
create_dhcpd_container() {
    local containerName="dhcpd_fixed4wan"
...
create_radiusd_container() {
    local containerName="radiusd_fixed4wan"
...
generate_frr_config() {
    local config_file="frr_fixed4wan.conf"
...
EOF
    ...
    echo "✅ FRR config written to $config_file for fixed 4-WAN uplinks"
    echo "--------------------------------------------------------------"
}
...
setup_nat() {
    ...
    sudo tee /usr/local/bin/setup-nat-fixed4wan.sh > /dev/null <<EOF
#!/bin/bash
set -euo pipefail
...
EOF
    ...
    # Make it executable
    sudo chmod +x /usr/local/bin/setup-nat-fixed4wan.sh
    
    # Run setup-nat-fixed4wan.sh
    echo "🔧 Setting up NAT with internet interface: $INTERNET_IFACE"
    /usr/local/bin/setup-nat-fixed4wan.sh
    
    echo "🔧 Creating NAT systemd service..."
    sudo tee /etc/systemd/system/setup-nat-fixed4wan.service > /dev/null <<EOF
[Unit]
Description=Fixed 4-WAN Network NAT and IP Forwarding Rules
After=network.target

[Service]
Type=oneshot
ExecStart=/usr/local/bin/setup-nat-fixed4wan.sh
RemainAfterExit=yes
...
EOF
    ...
    sudo systemctl enable setup-nat-fixed4wan.service
    sudo systemctl start setup-nat-fixed4wan.service
...
echo "\$(date): Starting Fixed 4-WAN DHCP service startup script" >> /var/log/startup.log
echo "\$(date): Starting Fixed 4-WAN DHCP service startup script"
...
EOF
    ...
    cat <<EOF > dhcpd.conf
# dhcpd.conf for Fixed 4-WAN Network Configuration
#
# Sample configuration file for ISC dhcpd
#

# option definitions common to all supported networks...
option domain-name "fixed4wan.local";
...
EOF
    ...
    echo "✅ Created dhcpd.conf with fixed 4-WAN uplink subnets"
    echo "--------------------------------------------------------------"
}
...
display_summary() {
    echo "✅ Fixed 4-WAN Network Setup Complete!"
    echo ""
    echo "==============================================================="
    echo "                    CONFIGURATION SUMMARY"
    echo "==============================================================="
    ...
    echo "   • frr_fixed4wan (FRR Routing - 4 interfaces)"
    echo "   • dhcpd_fixed4wan (DHCP Server)"
    echo "   • radiusd_fixed4wan (RADIUS Server)"
    echo ""
    ...
    echo ""
    echo "ℹ️  Use fixed_4_wan_cleanup.sh to remove all configuration when done"
}
...
echo "🔍 Starting Fixed 4-WAN Network configuration process..."
...
echo "🎉 Fixed 4-WAN Network setup completed successfully!"
